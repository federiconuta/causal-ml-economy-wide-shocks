...
for (month_name in months) {
  ...
  for (i in seq_len(n_splits)) {
    file_path <- file.path(split_input_dir, paste0("cates_s_", month_name, "_learner_split", i, ".RData"))
    
    if (file.exists(file_path)) {
      load(file_path)
      
      clan_results <- result_s_learner
...
vars_CAdiff <- c("cate_sl_test","Agriculture", "Chemicals", "Manufacturing", "Metals", "Special", "Textile", "Wood", "air", "land", "sea", "Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec","ND", "NO", "NP", "lnX", "lnX_import", "split")
...
for (sp in split_ids) {
  ...
  cates_sl <- data_split$cate_sl_test
...
write.csv(
  ...
  file = file.path(results_dir, "median_results_s_learn_ordered_normal.csv"),
  row.names = FALSE
)
...
write.csv(
  ...
  file = file.path(results_dir, "median_results_s_learn_ordered_industry.csv"),
  row.names = FALSE
)
...
write.csv(
  ...
  file = file.path(results_dir, "median_results_s_learn_ordered_industry_month.csv"),
  row.names = FALSE
)
