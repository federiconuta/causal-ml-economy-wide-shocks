...
# File: process_splits_by_month.R
...
for (month_name in months) {
  ...
  for (i in seq_len(n_splits)) {
    file_path <- file.path(split_input_dir, paste0("cates_dr_", month_name, "_learner_split", i, ".RData"))
    
    if (file.exists(file_path)) {
      load(file_path)
      
      clan_results <- result_dr_learner
...
vars_CAdiff <- c("cate_dr_test","Agriculture", "Chemicals", "Manufacturing", "Metals", "Special", "Textile", "Wood", "air", "land", "sea", "Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec","ND", "NO", "NP", "lnX", "lnX_import", "split")
...
for (sp in split_ids) {
  ...
  cates_sl <- data_split$cate_dr_test
...
write.csv(
  ...
  file = file.path(results_dir, "median_results_ordered_dr_learner_normal.csv"),
  row.names = FALSE
)
...
write.csv(
  ...
  file = file.path(results_dir, "median_results_ordered_dr_learner_industry.csv"),
  row.names = FALSE
)
...
write.csv(
  ...
  file = file.path(results_dir, "median_results_ordered_dr_learner_industry_month.csv"),
  row.names = FALSE
)
