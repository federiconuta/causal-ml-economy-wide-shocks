...
tab4_dir <- normalizePath(Sys.getenv("OBES_TAB4_DIR", script_dir), winslash = "/", mustWork = FALSE)
...
tab4_heter_dir <- normalizePath(Sys.getenv("OBES_TAB4_HETER_DIR", file.path(tab4_dir, "construction_data_heter")), winslash = "/", mustWork = FALSE)
...
load(file.path(sam_sum_dir, "bootstraps_mar26", "lasso.pred_SUM_boot_subsamp_cvlam_marzo26.RData"))    # Bootstrap predictions for SUM
load(file.path(sam_sum_dir, "bootstraps_mar26", "lasso.pred_SAM_boot_subsamp_cvlam_marzo26.RData"))    # Bootstrap predictions for SAM
...
original_predictions_lasso <- original_predictions_lasso %>% mutate(TE_indiv = pred_SAM - pred_SUM)
...
#Order each dataset by each bootstrap instead of by months 
lasso.pred_SAM_boot <- do.call(rlist::list.zip, lasso.pred_SAM_boot) %>%
  purrr::map(~ do.call(rbind, .))     # we have a list with as much as elements as bootstraps we have
lasso.pred_SUM_boot <- do.call(rlist::list.zip, lasso.pred_SUM_boot) %>%
  purrr::map(~ do.call(rbind, .))     # we have a list with as much as elements as bootstraps we have

lasso.pred_SAM_boot <- lapply(lasso.pred_SAM_boot, function(x) { #
  cbind(unique_id=rownames(x), x)
}) 

lasso.pred_SUM_boot <- lapply(lasso.pred_SUM_boot, function(x) { #
  cbind(unique_id=rownames(x), x)
}) 

#standardize the "pred" column in SAM and SUM datasets
new_col_name_SAM <- c("unique_id", "id", "month", "export_future", "pred")              
lasso.pred_SAM_boot <- lapply(lasso.pred_SAM_boot, setNames, nm = new_col_name_SAM) 
new_col_name_SUM <- c("unique_id", "month", "id", "export_future", "pred")
lasso.pred_SUM_boot <- lapply(lasso.pred_SUM_boot, setNames, nm = new_col_name_SUM) 

lasso.pred_SAM_boot <- bind_rows(lasso.pred_SAM_boot, .id = 'bootstrap') %>%  #convert a list of lists into a data frame, keeping the number of each list (bootstrap)
  mutate(covid_aware = 1)
lasso.pred_SUM_boot <- bind_rows(lasso.pred_SUM_boot, .id = 'bootstrap') %>%
  mutate(covid_aware = 0)


lasso.pred_SAM_boot <- lasso.pred_SAM_boot %>%
  rename(pred_SAM= pred)

lasso.pred_SUM_boot <- lasso.pred_SUM_boot %>%
  rename(pred_SUM = pred)


lasso_pred_boot <- inner_join(lasso.pred_SAM_boot, lasso.pred_SUM_boot, by = c("id", "bootstrap" ,"month"))

rm(lasso.pred_SAM_boot, lasso.pred_SUM_boot)




########################################################
# 6) MERGE SELECTED DATA WITH BOOTSTRAPS PREDICTIONS FROM LASSO
########################################################
data_heterogeneity_unique <- data_heterogeneity %>% select(-pred_lasso, -covid_aware) %>% unique()

library("tidyr")
data_heterogeneity_boots <- lasso_pred_boot %>%
  left_join(., data_heterogeneity_unique, by = c("id", "month"))  %>% tidyr::drop_na()   #drop na values
detach("package:tidyr", unload = TRUE)

#data_heterogeneity_boots <- lasso_pred_boot %>%
#  left_join(data_heterogeneity_unique, by = c("id", "month"))


write_csv(
  data_heterogeneity_boots,
  file.path(tab4_dir, "data_heterogeneity_boots.csv")
)
