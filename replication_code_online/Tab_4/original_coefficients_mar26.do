...
********************************* TABELLA 4 RIPRODUZIONE COEFFICIENTI ************************************

clear

local tab4_dir = subinstr("`c(pwd)'", "\", "/", .)
capture confirm file "data_heterogeneity_original.csv"
if _rc {
    capture cd "Tab_4"
    local tab4_dir = subinstr("`c(pwd)'", "\", "/", .)
}
...
import delimited "`tab4_dir'/data_heterogeneity_original.csv", clear 

save "`tab4_dir'/data_heterogeneity_original.dta", replace
...
rename te_indiv TE_indiv_original_SAM_SUM

*** **CODE IF WE WANT 25% value: (could be genralized)
centile (TE_indiv_original_SAM_SUM), centile (25 75) // search 25 and 75 percentiles
gen centiles = 25 if TE_indiv_original_SAM_SUM <=`r(c_1)'
replace centiles = 75 if TE_indiv_original_SAM_SUM >=`r(c_2)'
keep if centiles~=.
...
foreach v of varlist TE_indiv_original_SAM_SUM lnx lnx_import index_stringency_w index_stringency_w_import nd np Air Sea Land Jan Feb Mar Apr May Jun Jul Aug Sep Oct Nov Dec Agriculture Chemicals Manufacturing Metals Wood Special Textile{
	*local variables 
	*local not `v'
...
save "original_coefficients_advcadiff_nocontrol", replace
...
**Only the dummy coefficient (Coeff_orig) is needed, not the other regression coefficients.
keep id Variable Coeff_orig
save "original_coefficients_advcadiff_control_sec", replace
...
**Only the dummy coefficient (Coeff_orig) is needed, not the other regression coefficients.
keep id Variable Coeff_orig
save "original_coefficients_advcadiff_control_months_sec", replace
