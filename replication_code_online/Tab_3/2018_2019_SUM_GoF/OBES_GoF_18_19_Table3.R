...
calculate_gof <- function(preds, model_name, pred_col) {
  ...
  out <- data.frame(year = 19, month = months)
...
load(file.path(script_dir, "lasso_preds_18.RData"))
load(file.path(script_dir, "ridge_preds_18.RData"))
load(file.path(script_dir, "rf_preds_18.RData"))
load(file.path(script_dir, "logit_preds_18.RData"))

GoF_all <- Reduce(function(x, y) merge(x, y, by = c("year", "month"), all = TRUE), list(
  calculate_gof(lasso_preds_18, "lasso", "pred_18"),
  calculate_gof(ridge_preds_18, "ridge", "pred_18"),
  calculate_gof(rf_preds_18, "rf", "pred_18"),
  calculate_gof(logit_preds_18, "logit", "pred_18")
))
GoF_all <- GoF_all[order(as.numeric(as.character(GoF_all$month))), ]

write.csv(GoF_all, file.path(script_dir, "table3_gof_18_19_sum.csv"), row.names = FALSE)
print(GoF_all)
