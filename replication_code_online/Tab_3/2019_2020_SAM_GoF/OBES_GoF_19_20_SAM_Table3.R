...
load(file.path(script_dir, "lasso_preds_19.RData"))
load(file.path(script_dir, "ridge_preds_19.RData"))
load(file.path(script_dir, "rf_preds_19.RData"))
load(file.path(script_dir, "logit_preds_19.RData"))

GoF_all <- Reduce(function(x, y) merge(x, y, by = c("year", "month"), all = TRUE), list(
  calculate_gof(lasso_preds_19, "lasso", "pred_19"),
  calculate_gof(ridge_preds_19, "ridge", "pred_19"),
  calculate_gof(rf_preds_19, "rf", "pred_19"),
  calculate_gof(logit_preds_19, "logit", "pred_19")
))
GoF_all <- GoF_all[order(as.numeric(as.character(GoF_all$month))), ]

write.csv(GoF_all, file.path(script_dir, "table3_gof_19_20_sam.csv"), row.names = FALSE)
print(GoF_all)
